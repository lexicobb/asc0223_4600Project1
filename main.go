...
func SJFPrioritySchedule(w io.Writer, title string, processes []Process) {
	...
	// Sort the processes first by priority (higher priority first) and then by burst duration (shortest first)
...
func SJFSchedule(w io.Writer, title string, processes []Process) {
	...
	// Sort the processes by their burst duration (shortest first)
	sort.Slice(processes, func(i, j int) bool {
		return processes[i].BurstDuration < processes[j].BurstDuration
	})
...
func RRSchedule(w io.Writer, title string, processes []Process) {
	var (
		quantum     int64 = 1 // Default quantum value
		serviceTime int64
		//totalWait       float64
		totalTurnaround float64
		lastCompletion  float64
	...
	// Initialize remaining burst times and waiting times for all processes
	for i := range processes {
	...
	for {
		allFinished := true // Flag to check if all processes are finished

		for i := range processes {
			if remainingBurst[i] > 0 {
				allFinished = false // At least one process is not finished

				// Execute the process for a quantum or for its remaining burst time, whichever is shorter
				runTime := min(remainingBurst[i], quantum)
				...
				// If the process completed its burst, update its turnaround time
				if remainingBurst[i] == 0 {
					turnaround := runTime + waitingTime[i]
					totalTurnaround += float64(turnaround)
				}
	...
	count := float64(len(processes))
	aveWait := float64(sum(waitingTime)) / count
	aveTurnaround := totalTurnaround / count
	aveThroughput := count / lastCompletion
...
// Helper function to calculate the sum of an int64 slice
